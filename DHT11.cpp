...
void LCD_Display();       // 윈폼 UI에서 친 텍스트 LCD에 표기
...
void setup() { // 아두이노가 켜지거나 리셋될 때 딱 한 번만 실행되는 설정 구역
  ...
  // 9번 주소부터 15개의 방 개설 (첫 번째 줄용: 9 ~ 23)
  for (int i = 9; i < 19; i++) {
    slave.addHreg(i);
  }
  // 30번 주소부터 15개의 방 개설 (두 번째 줄용: 30 ~ 44)
  for (int i = 20; i < 30; i++) {
    slave.addHreg(i);
  }
  ...
  slave.Hreg(8, 0);
  // 첫 번째 줄 방(9~18)을 전부 공백(' ')으로 초기화
  for (int i = 9; i < 19; i++) {
    slave.Hreg(i, 32); // 32는 스페이스바(공백)의 아스키코드 숫자입니다.
  }

  // 두 번째 줄 방(20~29)을 전부 공백(' ')으로 초기화
  for (int i = 20; i < 30; i++) {
    slave.Hreg(i, 32);
  }
...
void loop() { // setup이 끝난 뒤, 아두이노가 꺼질 때까지 무한히 전속력으로 반복 실행되는 핵심 구역
  ...
  LCD_Display();    // 윈폼 UI에서 친 텍스트 LCD에 표기 함수 실행
  showEcho();
...
void LCD_Display() { // 윈폼 UI에서 친 텍스트 LCD에 표기 함수 정의
