...
void setup() { // 아두이노가 켜지거나 리셋될 때 딱 한 번만 실행되는 설정 구역
  ...
  slave.addCoil(2);    // 마스터가 ON/OFF할 수 있는 2번 Coil(비트 형태 창고) 개설 -> RGB LED 동시 제어용
  ...
  slave.Hreg(2, 3);    // 통신 연결 전 테스트용으로 2번 데이터 창고(조도)의 초기값을 숫자 3로 임시 저장
...
void SensorReading() { // 모든 센서의 전압 신호를 파악하고 가공하여 모드버스 통신 창고에 배정하는 함수 정의
  ...
  if (currentTime - lastTempHumiTime >= 1000) { 
...
void LED_Action() { // 모드버스 내부 창고에 저장된 데이터에 반응하여 아두이노 실제 LED 물리 핀을 제어하는 함수 정의
  ...
  // PC 마스터 프로그램이 2번 Coil 주소에 불을 켰는지(true) 검사하여 외부 RGB 모듈 제어 결정
  if (slave.Coil(2) > 0) { 
    analogWrite(R_LED, 255); // 참이면 9번 빨간색 LED 핀에 최대 아날로그 신호(255)를 주어 가장 밝게 켬
    analogWrite(G_LED, 255); // 참이면 10번 초록색 LED 핀에 최대 아날로그 신호(255)를 주어 가장 밝게 켬
    analogWrite(B_LED, 255); // 참이면 11번 파란색 LED 핀에 최대 아날로그 신호(255)를 주어 가장 밝게 켬 (셋이 뭉치면 백색광)
  } else { 
    analogWrite(R_LED, 0);   // 거짓(0)이면 9번 빨간색 LED 핀의 아날로그 전압 신호를 0으로 만들어 완전히 불을 끔
    analogWrite(G_LED, 0);   // 거짓(0)이면 10번 초록색 LED 핀의 아날로그 전압 신호를 0으로 만들어 완전히 불을 끔
    analogWrite(B_LED, 0);   // 거짓(0)이면 11번 파란색 LED 핀의 아날로그 전압 신호를 0으로 만들어 완전히 불을 끔
  } // RGB LED 제어 조건문 종료
} // LED_Action() 함수 종료
